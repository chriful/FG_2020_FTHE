...
def download_ckpt(outdir='data/fthe_data', use_3dpw=False):
    os.makedirs(outdir, exist_ok=True)

    if use_3dpw:
        ckpt_file = 'data/fthe_data/fthe_model_w_3dpw.pth.tar'
        url = 'https://www.dropbox.com/s/41ozgqorcp095ja/vibe_model_w_3dpw.pth.tar'
        if not os.path.isfile(ckpt_file):
            download_url(url=url, outdir=outdir)
    else:
        ckpt_file = 'data/fthe_data/fthe_model_wo_3dpw.pth.tar'
...
def prepare_rendering_results(fthe_results, nframes):
    frame_results = [{} for _ in range(nframes)]
    for person_id, person_data in fthe_results.items():
