...
def plot_accel(joints_pred, joints_gt, out_dir, name='', other_preds=['']):
    ...
    if other_preds:
        # accel_fthe = np.load(other_preds[1])[:len(time)]
        # accel_fthe = np.abs(accel_fthe - accel_gt)
        plt.plot(time, accel_gt * 1000, label='GT', color='#65D491')
        accel_fthe = np.load(other_preds[0])[:len(time)]
        # accel_fthe = np.abs(accel_fthe - accel_gt)
        plt.plot(time, accel_fthe * 1000, label='VFTHE', color='#3183F7') #75F74F '')

    accel_pred = compute_accel(joints_pred)
    # accel_err = np.abs(accel_pred - accel_gt)
    plt.plot(time, accel_pred*1000, label='Ours', color='#FF7F0E')
    ...
    out_plot_dir = f'./{out_dir}/plot_ours'
    Path(out_plot_dir).mkdir(parents=True, exist_ok=True)
    plot_name = f'./{out_plot_dir}/accel_error_{name}.png'
    print("...save plot to ", plot_name)
    plt.savefig(plot_name, bbox_inches = 'tight')
    np.save(f'./{out_plot_dir}/accel_{name}', accel_pred)
